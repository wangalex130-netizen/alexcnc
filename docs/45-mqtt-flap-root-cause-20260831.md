...
| 重启后 **列表仍显示 03 不在线** | 后端 `/api/machine/list` 的 `online` 字段 | ❌ **后端字段判定与实际情况不一致**，锅在后端 |
| 摄像头能看到 | 摄像头 → relay → App | ✅ 摄像头链路已通 |

→ **"列表不在线"与"MQTT 没连上"是两回事。现在可以定性：问题在后端 `online` 字段，不在 MQTT、也不在 App。**

**为什么"Jog 能动"反而证明了 MQTT 是好的**：
Jog 经 `cnc/<deviceId>/cmd` 下发，屏幕收到后真的动了 —— 说明 ①App 的 MQTT 连接与 ACL 正常；
②屏幕的 MQTT 订阅正常；③命令帧格式两端一致。这条链路上任何一环断了，Jog 都不会动。
所以"列表显示离线"只能是后端自己维护的在线状态没跟上。

**给 PC 后端（请回答）**：

1. `/api/machine/list` 的 `online` 字段**依据什么判定**？broker 在线客户端查询？LWT？还是心跳表？
2. 判定周期多久？屏幕重启后**多久**才会刷新成在线？
3. 建议：App 列表页用后端字段做粗筛，**进入机器后的控制台一律以 MQTT 状态帧为准** ——
   否则会出现"控制台明明能控制，列表却说离线"的自相矛盾，客户会以为机器坏了。
---
