...
### 5.3 现象 B（控制台显示"未连接"）的真因

**App 自身在闪断，订阅反复创建/销毁，导致大部分状态帧丢失**，于是控制台显示"未连接"。只要 P1 闪断修复（commit `62837545`，含于 APK `cb74b493`）生效，控制台就能正常显示坐标和状态。

> ✅ **澄清（2026-08-31 16:55，PC 后端已答复）**：`/api/machine/list` 只返回用户绑定过的设备与设备编号，**根本没有 `online` 字段**，阿里云后端仅业务层、不参与硬件连接。所以列表"在线/不在线"徽标**不可能来自后端**，只能由 App 本地推导。

### 5.4 机器"列表"显示不在线 —— 是**另一个独立 bug**，与闪断无关

**⚠️ 实测证伪旧结论**：新 APK `cb74b493`（已含闪断修复）装到真机后，**机器列表里 03 仍然显示"不在线"**，但 Jog/控制台能正常控制。这说明"列表不在线"**不是闪断的连带症状**——否则闪断修好后列表就该转在线。

**真正根因（已读代码确认）**：机器列表页 `machines_page.dart` 的"在线"徽标直接读 `machine.online`（来自后端 `Machine.fromJson`）。而后端**无此字段** → `machine.online` 恒为 `null` → 恒显示"不在线"。**这条判断逻辑压根没接 MQTT**，所以无论闪断修不修，列表永远是灰的。

| 表面现象 | 实际根因 | 是否随闪断修复解决 |
|---|---|---|
| 控制台显示"未连接"（现象 B） | App 闪断销毁订阅，状态帧丢失 | ✅ 是（commit `62837545`） |
| **机器列表 03 显示"不在线"** | 列表读后端 `machine.online`（null），未接 MQTT | ❌ 否 —— 独立显示 bug，本轮新修 |

**修复（2026-08-31 17:1x，本轮回合）**：新增 `deviceOnlineProvider`（按 sn 缓存本地在线态）+ `deviceOnlineTrackerProvider`（由实时状态帧 / 链路态写入，App 根 `AppShell` 常驻）。机器列表与固件页的"在线"徽标改为以该本地缓存为准：当前选中的机器有实时状态帧即显示"在线"，切走后标离线（诚实，App 确实只与一台机器建链）。后端侧**无需任何改动**（已确认无 online 字段）。

→ 请用**本轮新 APK** 重测：选中 03 并收到状态帧后，退回机器列表，03 应显示"在线"；切到别的机器或断开，03 转"不在线"。

**给 PC 后端（已答复，无需再问）**：
> `/api/machine/list` 无 `online` 字段，阿里云仅业务层、不参与硬件连接。后端侧无需为"在线状态"做任何改动；在线判定改由 App 本地连接态负责。
