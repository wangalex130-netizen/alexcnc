...
1. `mosquitto_sub -h 43.154.192.242 -p 8883 -u app-demo -P demo123 -t 'cnc/cnc-demo-04/status' -d` → SUBACK 成功（**不是 0x80**）；
2. `mosquitto_pub ... -t 'cnc/cnc-demo-04/cmd' -m '{"cmd":"hello"}'` → ACL deny 日志**无记录**；
3. App 选一台 `cnc-demo-04` 的机器 → 状态能进来、Jog 解锁、命令有响应。
