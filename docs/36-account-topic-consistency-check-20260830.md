...
> App 选择 `cnc-demo-04` 等白名单外机器时，界面显示「MQTT 已连接」，命令按钮也响应，但命令被 Broker 静默丢弃，**没有任何报错**。
...
| A2 | 037123 后台绑定表中，摄像头设备码登记为 `cnc-demo-03`（与机器同码） | 否则后端签发 token / 鉴权时会对不上 |
...
> 请一并确认：username 也用 `cam-cnc-demo-03`（不要 `cam-cnc-demo-01`），后台绑定表里摄像头设备码同为 `cnc-demo-03`。
>
> **③ 需要你确认/修正的清单（A 类阻塞，B 类本周内）**
> A1 测试账号绑定 `cnc-demo-03` 且 `/api/machine/list` 的 `code` 字段正确；
> A2 后台绑定表中摄像头设备码 = `cnc-demo-03`；
...
> B2 摄像头 username 改 `cam-cnc-demo-03`；
