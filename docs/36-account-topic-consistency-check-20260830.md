...
| 5 | 摄像头固件设备码烧录为该机器码 | ❌ **人工** | 摄像头 | ⚠️ **demo-03 已完成**（2026-08-30 刷机 + 端到端拉到 159KB 真实帧）；但**量产仍靠逐台改固件**（设备码在固件里硬编码三处）→ 须改 NVS / 设备自注册 |
...
## 十九、摄像头端切换 `cnc-demo-03` 完成核对（2026-08-30 14:30 收到回报）

### 19.1 我列的 10 项清单，逐条核对
...
| 8 | 心跳容忍 | 未回报 | ⚠️ 需确认仍忽略**非 `action` 字段**的帧（机器心跳 `{"cmd":"hello"}` 会进同一 `cmd` 主题） |
| 9 | relay 自愈 | 上电 `on=0` 待命 | ✅ 按需推流，未常推（`RELAY_MQTT_ENABLE=0` 的效果保住了） |
| 10 | 看门狗 | 未回报 | ⚠️ 待办：90s 无续租自动停推，防 App 被杀后常推 |

**端到端已闭环**：发 `stream_start` → 6 秒内中继拉到 **159009 字节**真实帧；`stream_stop` → 降到 2607。

### 19.2 App 侧核对结论（已查代码，非推测）
...
> **结论：登录态全链路已打通，App 零改动。**

### 19.3 ⚠️ 摄像头端有一处过时认知（必须纠正）

它说：「**`cnc-demo-03` 正好在 `app-demo` 的枚举白名单内**，所以 App 现在就能直接发流控指令，**不用改 ACL**」

- **事实**：`app-demo` 已于 **2026-08-30 13:35** 从枚举改为**通配** `cnc/+/...` 并部署上线。
- **「能用」这个结论是对的，但理由错了。**
- **危害**：它会误以为"新机器不在白名单就不能用"。真实情况是**任意新增设备码都已放行**
  （验收 `cnc/zzz-new-999/status` → ALLOW）。这个认知差会在讨论新机器时导致误判。

### 19.4 它自己挖出并修好的坑（各端都该知道）
...
| # | 事项 | 归属 |
|---|---|---|
| 1 | 确认 `cam-cnc-demo-01` 客户端已从 Dashboard 消失（重刷后应自动掉线）并清理 | MQTT |
| 2 | 演示模式拉流兜底 `cnc-demo-01` → 是否改为 `cnc-demo-03`（**待用户拍板**） | App |
| 3 | 摄像头固件 token 加默认值，防全擦 flash 复发 | 摄像头 |
| 4 | 设备码目前是固件**硬编码三处** → 量产需改 NVS / 设备自注册（第 5 道门） | 摄像头 |
| 5 | 摄像头 IP 现为 **`192.168.31.25`**（旧记录 `192.168.1.205` 已失效） | 记录更新 |

---

## 二十、订正：演示模式兜底这件事，我之前的分析不够准（2026-08-30 用户要求客观复核）

用户要求：「不再有演示模式，一切按实际来，正在接真机调试。客观看，不要说我想听的。」
复核了代码，结论：**我上一轮给的方案 A 结论对，但理由不准确；而且 A/B 都不是最优解。**

### 20.1 先把 App 的三种状态拆清楚（读代码得出，非推测）

| 状态 | 拉流设备码 | MQTT 控制设备码 |
|---|---|---|
| 真实模式 + 已登录 + **已选机器** | `machine.sn`（**后端返回的 `code`**） | 同左 |
| 真实模式 + 已登录 + **未选机器** | 控制台页 → 硬编码 `cnc-demo-01`<br>全屏页 → 空（`if (realMode) return ''`） | 硬编码 `cnc-demo-01`（`AppConfig.DEVICE_ID`） |
| 真实模式 + **未登录** | 两页均返回空 | 空 |
| 演示模式（`useRealBackend=false`） | 硬编码 `cnc-demo-01` | Mock，**不发真实命令** |

依据：`fullscreen_preview_page.dart:60-79`、`console_page.dart:182-187`、`config.dart:40-43/96`。

### 20.2 订正我上一轮的说法

| 我上一轮说的 | 实际情况 |
|---|---|
| 「演示模式会拉空」 | ✅ 对（演示模式确实拉 `/stream/cnc-demo-01`，摄像头已切 03） |
| 「改成 03 会造成『控 01、看 03』不一致」 | ❌ **不准确**。演示模式走 Mock，根本不发真实 MQTT 命令，不存在"控 01"；只是单纯拉不到流 |
| 「演示模式（未登录）」 | ⚠️ **混淆了两个概念**。真实模式下未登录一律返回空；硬编码兜底只在**演示模式**和**真实+已登录+未选机器**时生效 |

### 20.3 真正重要的发现：A1 至今没被确认，而两处记录互相矛盾

摄像头端这次「切 03」的**全部前提**是「测试账号绑的是 `cnc-demo-03`」。但：

| 来源 | 写的是 |
|---|---|
| App 代码注释（`machines_service.dart:122`） | 测试账号 `3020 Nova` 已预绑定 **`cnc-demo-01`** |
| 本文档 §十四 A1 / §十五 ② | 测试账号绑定的是 **`cnc-demo-03`** |

**这不是小分歧**：

- 若实际是 **`cnc-demo-03`** → 现在一切正确，切 03 切对了；
- 若实际是 **`cnc-demo-01`** → **这次切 03 把联调切反了**：登录后 App 用 `sn=cnc-demo-01`，
  拉 `/stream/cnc-demo-01`（空）、控 `cnc/cnc-demo-01/cmd`（摄像头收不到），
  而摄像头却在 `cnc-demo-03` 上待命。

**我尝试直接实测后端验证，失败**：`POST https://037123.xyz/api/auth/android/login`
返回 **HTTP 403 / Cloudflare `error code: 1010`**（反爬拦截，非服务端逻辑错误）。
→ **只能由人在手机/浏览器上登录，或让 PC 工程师直接给 `/api/machine/list` 的原始 JSON。**

### 20.4 另一个真实缺陷（与 A/B 无关，但确实存在）

**真实模式 + 已登录 + 未选机器**时：

- 全屏预览页：✅ 已挡住（返回空，不会拉）
- **控制台页：❌ 没挡** —— `_resolvedRelayUrl()` 在 `currentMachineProvider == null` 时
  直接回退到硬编码 `cnc-demo-01`，**静默指向一台可能不存在的机器**。

### 20.5 结论：方案 C（我改主意了，A 和 B 都不对）

| 方案 | 评价 |
|---|---|
| A：保持 `cnc-demo-01` | 结论对但理由错；且缺陷（20.4）仍在 |
| B：改成 `cnc-demo-03` | ❌ **只是换了个错法**。把硬编码机器码从 01 换成 03，隐患一模一样——量产包里每个用户都默认指向同一台机器 |
| **C（建议）** | ① **不换数字**；② **删掉控制台页的兜底**，与全屏页对齐：真实模式下未选机器返回空 + 提示"请先选择机器"；③ 演示模式既然退役，硬编码 `cnc-demo-01` 兜底连同 `AppConfig.DEVICE_ID` 的默认值**整体清除** |
