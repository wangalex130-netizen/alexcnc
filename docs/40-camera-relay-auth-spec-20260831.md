...
7. **🆕 摄像头推流端**：是否一并签发「设备级 publish token」？有效期、吊销方式、配网时如何下发？**详见 §12**（这条与摄像头量产方案是同一次改造，建议一起定）
...
## 12. 🆕 补充（2026-08-31）：**摄像头推流端**的 token 也要管

> 本节是初版遗漏，写完核对时发现，务必一并纳入设计。

初版只覆盖了**拉流端**（App → `/stream/<device>`）。但中继还有**推流端**：

```
摄像头 ──HTTP POST─▶ /publish/<device>?token=<token>&user=<user>
```

**问题**：摄像头现在用的是**全局共享 token**。若 `/stream` 按账号绑定严格校验，
而 `/publish` 仍用一个所有人共用的 token，则任何人拿到该 token 就能
**往任意设备码上推假画面**（虽然危害小于被偷看，但属于伪造 / 占位攻击，
且能让客户 App 上出现别人的画面）。

**要求**：`/publish/<device>` 同样按 **设备维度** 校验，token 与设备码一一绑定。

### 12.5.1 摄像头如何拿到自己的 publish token（推荐路径）

摄像头**不登录、不持有账号**，因此不能走「登录后下发」。推荐复用已定的量产方案：

```
产线/配网时：屏幕配网，密码与设备码经 UART 下发给摄像头（方案 C，docs/39 §七）
                    ↓ 同一通道顺带下发
              摄像头 publish token（每台唯一）
                    ↓
              摄像头存 NVS，推流时带上
```

**好处**：与设备码下发走同一条已确定的通道，边际成本极低；
且天然满足「屏幕码 = 摄像头码 = 推流凭据」三者同源。

### 12.5.2 请 PC 端一并确认

| # | 问题 | 建议 |
|---|---|---|
| 1 | 是否同时签发「设备级 publish token」（与账号无关、长期有效）？ | **是**，必须签 |
| 2 | publish token 的有效期？ | 建议长期（1 年+）或不过期，但**可按设备吊销** |
| 3 | 由谁在配网时把它交给屏幕？ | 阿里云后端 → 屏幕（配网流程里已联网）→ UART → 摄像头 |
| 4 | 设备换机 / 重新配网时旧 token 是否作废？ | 建议**重新配网即轮换**，旧 token 立即失效 |
| 5 | 是否提供「按设备吊销 publish token」的运维接口？ | 建议提供，用于丢机/二手转让场景 |

### 12.5.3 涉及的其他端

| 端 | 需要做什么 |
|---|---|
| **中继 relay.py** | `/publish/<device>` 按设备绑定校验 token；与 `/stream` 用同一套绑定表 |
| **屏幕固件** | 配网时除 `ssid / pass / device` 外，向摄像头多下发一个 `relay_publish_token` 字段 |
| **摄像头固件** | 从 NVS 读取 publish token（替代现有硬编码默认值，与方案 C 同一次改造） |
| **阿里云后端** | 提供设备级 publish token 的签发与吊销 |

> ⚠️ **排期提示**：这一条与摄像头「方案 C（设备码配网下发）」是**同一次固件改造**，
> 顺手一起做成本最低；等方案 C 落地后再补，等于让摄像头再烧一次固件。

---

## 13. 变更摘要（可转发）

> **A. 拉流端（App 看画面）— PC 后端需要新增**
> 1. 登录/机器列表接口给每台机器返回 `relayToken` + `relayTokenExpiresAt`。
> 2. token 按 `userId + devices[]` 签发，设备码不保密，鉴权靠 token。
> 3. 同步 JWT 签名密钥给中继。
>
> **B. 推流端（摄像头传画面）— 需要一并设计（§12，初版遗漏，补充）**
> 4. 签发**设备级 publish token**（每台唯一，与账号无关，可吊销）。
> 5. 配网时由屏幕经 UART 下发给摄像头（复用方案 C 的同一条通道，边际成本极低）。
> 6. 提供按设备吊销 publish token 的运维接口（丢机 / 二手转让场景）。
>
> **中继（relay.py）需要**
> 1. 开启 `REQUIRE_BINDING`。
> 2. `/stream/<device>` 按「账号 + 设备绑定」校验；`/publish/<device>` 按「设备绑定」校验。
