...
| # | 反馈 | 处理 |
|---|---|---|
| 1 | 「Jog 里的 XY 和 Z 是干吗的？」 | **Z 是真 bug**：它是个按钮，点击会下发 `jog('z', 0)`（移动 0mm 的空命令），还会刷新固件 Feed Hold 计时器；而 XY 只是不可点的装饰标签。两者外观却几乎一样。已统一改为纯标签 `_axisLabel()`（无边框、更淡），**两者都不可点** |
| 2 | 「显示演示模式是不是因为机器没在线？」 | **不是**。演示模式 = App 的「真实设备连接」开关关闭（`useRealBackend=false`），与机器是否开机、是否在线**完全无关**。为避免误解：新增醒目黄色横幅，明确写「与机器是否开机无关」，并给出「开启真实设备连接」按钮直达联调设置页；同时把该页开关文案从「启用真实后端 (Real)」改为「启用真实设备连接 / 关闭则只演示：命令不会下发给机器」 |
