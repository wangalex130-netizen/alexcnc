...
| `softReset()` | `{"cmd":"reset"}` | **软复位 = Grbl `Ctrl-X`(0x18)**：立即中止当前运动、清空规划器缓冲。**不移动任何轴**。 |
| `unlock()` | `{"cmd":"unlock"}` | **解除报警锁定 = Grbl `$X`**：只清 Alarm/Lock 位，回到可运动状态。**不回零、不移动**；解锁后工件坐标不可信，须重新定原点。 |
...
#### 🔴 2.2.1 `reset` / `unlock` 的强制语义（固件必须照做，2026-08-31 新增）

这两个命令是**报警自救通道**，App 侧刻意让它们**不受 `state == idle` 闸门限制**
（机器报警后 Jog/回零全被锁死，若解锁也一起锁，App 就永远救不回报警，用户只能跑去按机器上的实体键）。
正因为 App 放开了闸门，**固件侧的语义必须严格、不得越界**：

| 命令 | 固件必须做 | 固件**禁止**做 |
|---|---|---|
| `{"cmd":"reset"}` | 等价 Grbl `Ctrl-X`(0x18)：中止运动、清空规划器缓冲、重置 Grbl 到可接收状态 | ❌ 不得移动任何轴；❌ 不得自动开始加工；❌ 不得自动回零 |
| `{"cmd":"unlock"}` | 等价 Grbl `$X`：清 Alarm/Lock 位，使机器回到可运动状态 | ❌ 不得回零；❌ 不得移动；❌ 不得清除**用户已设的工件零点**（G54 保持不动，但坐标可信度由用户自行判断） |

**为什么必须这么严**：解锁后机器立刻能运动，而此时坐标系可能已经失效（撞刀/限位/掉步都会让 Grbl 失去位置参考）。
App 侧在点「解锁」后会弹提示「已解除报警锁定 · 请重新定原点后再加工」，
把判断权交给用户；固件只要做了"顺手回零"这类额外动作，就等于替用户做了假设，容易出安全事故。

**状态回报要求**：

1. 收到 `unlock` 并成功清锁后，状态帧 `state` 应回到 `idle`、**不要**继续发 `alarm`；
2. 收到 `reset` 后同样回 `idle`；
3. 若清锁失败（例如硬限位仍处于触发状态），**保持 `alarm` 并把原因写进 `alarm_code`**，
   让 App 知道"解锁没成功"，不要假装成功回 `idle`（会导致 Jog 被错误解锁）。
4. 这两个命令同样要刷新 Feed Hold 心跳计时器（与 `cnc_mqtt_dispatch()` 里其他命令一致）。

> ⚠️ 固件若暂未实现这两个命令，**请先明确回复**，App 侧按钮会表现为"点了没反应"。
> 在此之前 App 的软复位/解锁只发命令、不校验结果，属于"尽力而为"。
