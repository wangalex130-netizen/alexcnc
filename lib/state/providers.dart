...
class ActiveJob {
  ...
  /// 用户已确认过「加工完成」弹窗；确认后不再重复弹出。
  final bool completedAcknowledged;
  ...
  ActiveJob copyWith({
    ...
    bool? completedAcknowledged,
      ...
      ActiveJob(
        ...
        completedAcknowledged: completedAcknowledged ?? this.completedAcknowledged,
      );
}
...
class ActiveJobNotifier extends StateNotifier<ActiveJob?> {
  ...
  /// 用户已确认「加工完成」弹窗；确认后不再重复弹出。
  void acknowledgeCompletion() {
    final job = state;
    if (job == null || !job.completed) return;
    state = job.copyWith(completedAcknowledged: true);
  }

  /// 清除已完成的加工记录（不触发 stopJob，仅 UI 归档）。
