...
///
/// 顶部视频监控（含右上角延时摄影开关）；快捷开关（机箱照明/红点激光）；
...
class _ConsolePageState extends ConsumerState<ConsolePage>
  ...
  /// 两级交互第一级：延时摄影「功能已打开」但尚未开始采样。
  /// 为 true 时右上角图标亮起「已开启」，画面内出现「开始录制」按钮；
  /// 点「开始录制」才真正调 start()（第二级）。与雕刻态向导入口共用 jobId。
  bool _tlArmed = false;
  ...
  /// 右上角「延时摄影」图标（两级交互的第一级 = 打开功能 / 查看视频）。
  ///
  /// 场景语义：
  /// - 非雕刻态：点击图标 = 打开延时摄影功能（_tlArmed=true，画面出现「开始录制」）；
  ///   再点「开始录制」才真正采样；录制中点「停止」→ 服务器自动拼接生成。
  /// - 雕刻态：向导在雕刻开始时写入 jobId（running），本页图标自动变「延时采集中」，
  ///   雕刻结束服务器自动停止拼接保存，图标变绿色「已生成」，点击直接打开视频观看。
  /// - 与向导 Step6 共用同一 timeLapseJobProvider，故两端状态一致、可互看视频。
  Future<void> _toggleTimeLapse() async {
    final jobId = _tlJobId;
    if (jobId != null && _isTlReady()) {
    ...
    // 无进行中的 job：第一级 = 打开/关闭延时功能（仅本地开关，不真正采样）。
    setState(() => _tlArmed = !_tlArmed);
  }

  /// 两级交互第二级：「开始录制」按钮 —— 仅在延时功能已打开(_tlArmed)且无 job 时可用。
  /// 点击真正调 start()，服务器开始按采样间隔抽帧存图。
  Future<void> _tlStartRecording() async {
    if (_tlJobId != null) return; // 已有 job 进行中，忽略。
    final id = await TimeLapseClient.start(durationSec: 120);
    if (id != null) ref.read(timeLapseJobProvider.notifier).setJob(id);
  }

  /// 两级交互第二级：「停止」按钮 —— 录制中点击停止并触发自动拼接生成。
  ...
  /// 是否显示「开始录制 / 停止」浮动按钮：
  /// 延时功能已打开（armed）但未采样 → 显示「开始录制」；
  /// 正在采样（running）→ 显示「停止」。
  bool _tlShowRecordBtn() => (_tlArmed && _tlJobId == null) || _isTlRunning();
  ...
  Color _tlColor() {
    ...
    if (_isTlReady() || _tlArmed) return CncColors.primary;
  ...
  void _togglePauseResume(MachineState state) {
    final hw = ref.read(hardwareServiceProvider);
    if (state != MachineState.busy && state != MachineState.paused) {
      _showHint('当前不在加工中，无需暂停/继续');
      return;
    }
        ...
        children: [
          ...
          Stack(
            children: [
              ...
              // 延时摄影「开始录制 / 停止」浮动按钮：两级交互的第二级。
              // 延时功能已打开（armed）但未采样 → 绿色「开始录制」；
              // 正在采样（running）→ 红色「停止」并自动拼接。雕刻态雕刻开始后自动进入 running。
              if (_tlShowRecordBtn())
                Positioned(
                  left: 0,
                  right: 0,
                  ...
                  child: Center(
                    child: GestureDetector(
                      onTap: _isTlRunning() ? _tlStopRecording : _tlStartRecording,
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 8),
                        decoration: BoxDecoration(
                          color: _isTlRunning()
                              ? CncColors.danger.withOpacity(0.9)
                              : CncColors.primary.withOpacity(0.9),
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(
                              _isTlRunning() ? Icons.stop_rounded : Icons.fiber_manual_record_rounded,
                              size: 14,
                              color: Colors.black,
                            ),
                            const SizedBox(width: 6),
                            Text(
                              _isTlRunning() ? '停止录制' : '开始录制',
              ...
              Positioned(
                ...
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    // 延时摄影开关：收进右上角一个图标，不再占下面一整排。
                    // 未开=静默；采集中=红点+「延时采集中」；已生成=绿色「已生成」。
                    GestureDetector(
              ...
              // LAN (RTSP) 模式不显示——画面内操作已收敛到左下角暂停/停止。
              if (!isLocal)
                Positioned(
                  right: 10,
                  bottom: 8,
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                              ...
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: const Icon(Symbols.close,
                                      size: 16, color: CncColors.primaryInk),
                                ),
                              ),
                ...
                // 延时摄影状态卡：采集中不显示（避免画面下方出现「延时摄影中」一排框干扰），
                // 仅生成完成（video_ready）或失败后给出「查看 / 下载」入口。
                // 与向导 Step6 共用 timeLapseJobProvider，故 carve 联动或本页手动开启都在此呈现。
                if (tlJobId != null && _tlShowCard())
              ...
              children: [
                ...
                Expanded(
                  child: _ActionBtn(
                    ...
                    onTap: () => _togglePauseResume(status.state),
                  ),
                ),
              ],
            ),
