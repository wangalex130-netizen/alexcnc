...
class MachineStatus {
  ...
  factory MachineStatus.fromJson(Map<String, dynamic> j) {
    ...
    final s = (j['state'] ?? 'idle').toString().toLowerCase();
    // 固件可能用 'run'/'running' 表示正在运行；PROTOCOL 约定为 'busy'。
    // jobState 也作为辅助依据。
    final jobState = (j['jobState'] ?? '').toString().toLowerCase();
    const _stateAlias = <String, MachineState>{
      'run': MachineState.busy,
      'running': MachineState.busy,
      'hold': MachineState.paused,
    };
    final alias = _stateAlias[s];
    if (alias != null) {
      state = alias;
    } else if (s == 'running' || jobState == 'running') {
    ...
    // 防御性归一化：若大于 1 则视为百分比并缩放，避免一启动就显示 100%。
    if (!prog.isNaN && prog > 1.0) {
      prog = prog / 100.0;
    }
